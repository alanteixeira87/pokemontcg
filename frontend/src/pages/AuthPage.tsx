    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <header className="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
        <div className="mx-auto max-w-5xl px-4 py-5">
          <p className="text-xs font-medium uppercase tracking-[0.16em] text-indigo-600 dark:text-indigo-300">Pokemon TCG</p>
          <h1 className="text-2xl font-semibold text-slate-950 dark:text-white">Colecao Local</h1>
        </div>
      </header>

      <main className="mx-auto grid min-h-[calc(100vh-82px)] max-w-5xl items-center gap-6 px-4 py-8 lg:grid-cols-[1fr_420px]">
        <section>
          <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium uppercase text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300">Colecao privada</span>
          <h2 className="mt-4 text-4xl font-semibold leading-tight tracking-tight text-slate-950 dark:text-white">Entre para salvar suas cartas na sua propria conta.</h2>
        ...
        <form onSubmit={submit} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-5">
            <h3 className="text-xl font-semibold text-slate-950 dark:text-white">{mode === "login" ? "Entrar" : "Criar conta"}</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400">Use e-mail e senha para acessar sua colecao.</p>
          </div>
          ...
          <button
            type="button"
            className="mt-4 w-full text-center text-sm font-medium text-indigo-600 transition hover:text-indigo-700 dark:text-indigo-300"
